5114
((3) 0 () 13 ((c (? . 1) q hyper-panel-mixin) (q lib "browser/browser.rkt") (c (? . 1) q hyper-text-mixin) (q lib "browser/external.rkt") (q 7582 . 3) (q 7675 . 4) (c (? . 1) q hyper-frame<%>) (c (? . 1) q hyper-canvas-mixin) (c (? . 1) q image-map-snip%) (c (? . 1) q hyper-frame-mixin) (q lib "browser/htmltext.rkt") (c (? . 10) q html-text<%>) (c (? . 1) q hyper-text<%>)) () (h ! (equal) ((c def c (c (? . 3) q install-help-browser-preference-panel)) q (10018 . 2)) ((c constructor c (? . 0)) q (5595 . 6)) ((c meth c (c (? . 0) q reload)) q (6620 . 2)) ((c meth c (c (? . 2) q make-link-style)) q (2214 . 5)) ((c meth c (c (? . 7) q get-editor%)) q (4690 . 2)) ((c meth c (c (? . 6) q get-hyper-panel%)) q (316 . 2)) ((c def c (c (? . 1) q html-img-ok)) q (102 . 4)) ((c def c (c (? . 1) q make-exn:cancelled)) c (? . 4)) ((c constructor c (? . 2)) q (1434 . 10)) ((c def c (c (? . 1) q hyper-text%)) q (4414 . 3)) ((c def c (c (? . 3) q update-browser-preference)) q (9927 . 3)) ((c meth c (c (? . 2) q add-link)) q (3258 . 5)) ((c def c (c (? . 1) q hyper-no-show-frame%)) q (838 . 3)) ((c meth c (c (? . 2) q eval-racket-string)) q (4027 . 3)) ((c meth c (c (? . 2) q hyper-delta)) q (2622 . 2)) ((c meth c (c (? . 2) q remove-tag)) q (2965 . 3)) ((c meth c (c (? . 8) q set-key)) q (7210 . 3)) ((c meth c (c (? . 11) q post-url)) q (9161 . 5)) ((c def c (c (? . 1) q exn:file-saved-instead-pathname)) c (? . 5)) ((c meth c (c (? . 2) q reload)) q (4109 . 2)) ((c def c (c (? . 1) q struct:exn:cancelled)) c (? . 4)) ((c meth c (c (? . 2) q add-racket-callback)) q (3438 . 7)) ((c def c (c (? . 1) q make-exn:file-saved-instead)) c (? . 5)) ((c meth c (c (? . 2) q get-hyper-keymap)) q (4261 . 2)) ((c meth c (c (? . 12) q url-allows-evalling?)) q (1205 . 3)) ((q form ((lib "browser/browser-sig.rkt") browser^)) q (7849 . 2)) ((c meth c (c (? . 2) q get-url)) q (2408 . 3)) ((c def c (c (? . 1) q bullet-size)) q (6899 . 4)) ((q def ((lib "browser/browser-unit.rkt") browser@)) q (7824 . 2)) ((c meth c (c (? . 2) q post-url)) q (3043 . 5)) ((c meth c (c (? . 2) q remap-url)) q (4156 . 3)) ((c def c (c (? . 1) q exn:file-saved-instead?)) c (? . 5)) ((c def c (c (? . 1) q open-url)) q (0 . 3)) ((c def c (c (? . 1) q exn:file-saved-instead)) c (? . 5)) ((c def c (? . 9)) q (459 . 4)) ((c meth c (c (? . 2) q add-tag)) q (2681 . 4)) ((c meth c (c (? . 2) q set-title)) q (2547 . 3)) ((c def c (c (? . 1) q hyper-frame%)) q (1052 . 4)) ((c meth c (c (? . 6) q get-hyper-panel)) q (390 . 2)) ((c def c (? . 7)) q (4492 . 3)) ((c def c (c (? . 1) q exn:cancelled)) c (? . 4)) ((c def c (? . 8)) q (7020 . 3)) ((c def c (c (? . 1) q html-eval-ok)) q (188 . 4)) ((c def c (c (? . 1) q page->editor)) q (6827 . 3)) ((c def c (c (? . 3) q send-url)) q (9752 . 4)) ((c constructor c (? . 9)) q (590 . 6)) ((c meth c (c (? . 0) q rewind)) q (6135 . 2)) ((c meth c (c (? . 2) q find-tag)) q (2800 . 4)) ((c meth c (c (? . 8) q get-key)) q (7288 . 2)) ((c constructor c (? . 8)) q (7076 . 4)) ((c def c (c (? . 1) q hyper-panel%)) q (6668 . 3)) ((c def c (? . 0)) q (5453 . 4)) ((c meth c (c (? . 8) q add-area)) q (7343 . 7)) ((c def c (c (? . 3) q add-to-browser-prefs-panel)) q (10079 . 3)) ((c meth c (c (? . 2) q get-title)) q (2495 . 2)) ((c meth c (c (? . 0) q make-control-bar-panel)) q (6013 . 3)) ((c def c (c (? . 1) q hyper-panel<%>)) q (5413 . 2)) ((c def c (c (? . 10) q render-html-to-text)) q (9463 . 9)) ((c meth c (c (? . 0) q filter-notes)) q (6515 . 3)) ((c def c (c (? . 10) q html-text-mixin)) q (9374 . 3)) ((q def ((lib "browser/tool.rkt") tool@)) q (10181 . 2)) ((c constructor c (? . 7)) q (4593 . 3)) ((c meth c (c (? . 11) q set-title)) q (8015 . 3)) ((c meth c (c (? . 0) q on-navigate)) q (6304 . 2)) ((c meth c (c (? . 7) q after-set-page)) q (5356 . 2)) ((c meth c (c (? . 0) q leaving-page)) q (6357 . 5)) ((c def c (c (? . 1) q editor->page)) q (6751 . 3)) ((c meth c (c (? . 11) q add-tag)) q (8268 . 4)) ((c def c (? . 12)) q (1166 . 2)) ((c def c (? . 11)) q (7882 . 3)) ((c meth c (c (? . 11) q make-link-style)) q (8386 . 5)) ((c meth c (c (? . 2) q add-thunk-callback)) q (3745 . 7)) ((c meth c (c (? . 11) q get-url)) q (7943 . 2)) ((c meth c (c (? . 7) q set-page)) q (5249 . 4)) ((c meth c (c (? . 0) q get-canvas%)) q (5935 . 3)) ((c meth c (c (? . 2) q map-shift-style)) q (1918 . 7)) ((c meth c (c (? . 0) q make-canvas)) q (5824 . 3)) ((c meth c (c (? . 7) q current-page)) q (4759 . 2)) ((c meth c (c (? . 7) q goto-url)) q (4814 . 9)) ((c def c (c (? . 1) q struct:exn:file-saved-instead)) c (? . 5)) ((c meth c (c (? . 11) q add-thunk-callback)) q (8882 . 7)) ((c def c (? . 2)) q (1306 . 4)) ((c meth c (c (? . 0) q forward)) q (6183 . 2)) ((c meth c (c (? . 11) q add-racket-callback)) q (8578 . 7)) ((c def c (c (? . 1) q hyper-canvas%)) q (4331 . 3)) ((c def c (? . 6)) q (276 . 2)) ((c meth c (c (? . 0) q get-canvas)) q (6232 . 2)) ((c meth c (c (? . 11) q add-link)) q (8089 . 5)) ((c def c (c (? . 1) q hyper-no-show-frame-mixin)) q (952 . 3)) ((c def c (c (? . 3) q browser-preference?)) q (9862 . 3)) ((c def c (c (? . 1) q exn:cancelled?)) c (? . 4))))
procedure
(open-url url) -> (is-a?/c hyper-frame%)
  url : (or/c url? string? input-port?)
parameter
(html-img-ok) -> boolean?
(html-img-ok ok?) -> void?
  ok? : any/c
parameter
(html-eval-ok) -> boolean?
(html-eval-ok ok?) -> void?
  ok? : any/c
interface
hyper-frame<%> : interface?
method
(send a-hyper-frame get-hyper-panel%) -> (subclass?/c panel%)
method
(send a-hyper-frame get-hyper-panel) -> (is-a?/c panel%)
mixin
hyper-frame-mixin : (class? . -> . class?)
  argument extends/implements: frame%
  result implements: hyper-frame<%>
constructor
(new hyper-frame-mixin                        
                       [url url]              
                       ...superclass-args...) 
 -> (is-a?/c hyper-frame-mixin)
  url : (or/c url? string? input-port?)
class
hyper-no-show-frame% : class?
  superclass: (hyper-frame-mixin (frame:status-line-mixin frame:basic%))
mixin
hyper-no-show-frame-mixin : (class? . -> . class?)
  argument extends/implements: frame%
class
hyper-frame% : class?
  superclass: (hyper-no-show-frame-mixin (frame:status-line-mixin
frame:basic%))
interface
hyper-text<%> : interface?
method
(send a-hyper-text url-allows-evalling? url) -> boolean?
  url : (or/c port? url?)
mixin
hyper-text-mixin : (class? . -> . class?)
  argument extends/implements: text%
  result implements: hyper-text<%>
constructor
(new hyper-text-mixin                             
                      [url url]                   
                      [status-frame status-frame] 
                      [post-data post-data]       
                      ...superclass-args...)      
 -> (is-a?/c hyper-text-mixin)
  url : (or/c url? string? input-port?)
  status-frame : (or/c (is-a?/c top-level-window<%>) false/c)
  post-data : (or/c false/c bytes?)
method
(send a-hyper-text map-shift-style start            
                                   end              
                                   shift-style) -> void?
  start : exact-nonnegative-integer?
  end : exact-nonnegative-integer?
  shift-style : style<%>
method
(send a-hyper-text make-link-style start     
                                   end)  -> void?
  start : exact-nonnegative-integer?
  end : exact-nonnegative-integer?
method
(send a-hyper-text get-url)
 -> (or/c url? string? input-port? false/c)
method
(send a-hyper-text get-title) -> string?
method
(send a-hyper-text set-title str) -> void?
  str : string?
method
(send a-hyper-text hyper-delta) -> style-delta%
method
(send a-hyper-text add-tag name pos) -> void?
  name : string?
  pos : exact-nonnegative-integer?
method
(send a-hyper-text find-tag name/number)
 -> (or/c exact-nonnegative-integer? false/c)
  name/number : (or/c string? exact-nonnegative-integer?)
method
(send a-hyper-text remove-tag name) -> void?
  name : string?
method
(send a-hyper-text post-url  url                   
                            [post-data-bytes]) -> void?
  url : (or/c string? url?)
  post-data-bytes : (or/c bytes? false/c) = #f
method
(send a-hyper-text add-link start end url) -> void?
  start : exact-nonnegative-integer?
  end : exact-nonnegative-integer?
  url : (or/c url? string?)
method
(send a-hyper-text add-racket-callback start            
                                       end              
                                       racket-expr) -> void?
  start : exact-nonnegative-integer?
  end : exact-nonnegative-integer?
  racket-expr : string?
method
(send a-hyper-text add-thunk-callback start      
                                      end        
                                      thunk) -> void?
  start : exact-nonnegative-integer?
  end : exact-nonnegative-integer?
  thunk : (-> any)
method
(send a-hyper-text eval-racket-string str) -> any
  str : string?
method
(send a-hyper-text reload) -> void?
method
(send a-hyper-text remap-url url) -> (or/c url? string?)
  url : (or/c url? string?)
method
(send a-hyper-text get-hyper-keymap) -> (is-a?/c keymap%)
class
hyper-canvas% : class?
  superclass: (hyper-canvas-mixin canvas:basic%)
class
hyper-text% : class?
  superclass: (hyper-text-mixin text:keymap%)
mixin
hyper-canvas-mixin : (class? . -> . class?)
  argument extends/implements: editor-canvas%
constructor
(new hyper-canvas-mixin ...superclass-args...)
 -> (is-a?/c hyper-canvas-mixin)
method
(send a-hyper-canvas get-editor%) -> (subclass?/c text%)
method
(send a-hyper-canvas current-page) -> any/c
method
(send a-hyper-canvas goto-url  url                 
                               relative-to-url     
                              [progress-proc       
                               post-data])     -> void?
  url : (or/c url? string?)
  relative-to-url : (or/c url? string? false/c)
  progress-proc : (boolean? . -> . any) = void
  post-data : (or/c bytes? false/c) = #f
method
(send a-hyper-canvas set-page page notify?) -> void?
  page : any/c
  notify? : any/c
method
(send a-hyper-canvas after-set-page) -> void?
interface
hyper-panel<%> : interface?
mixin
hyper-panel-mixin : (class? . -> . class?)
  argument extends/implements: area-container<%>
  result implements: hyper-panel<%>
constructor
(new hyper-panel-mixin                         
                       [info-line? info-line?] 
                       ...superclass-args...)  
 -> (is-a?/c hyper-panel-mixin)
  info-line? : any/c
method
(send a-hyper-panel make-canvas container) -> void?
  container : (is-a?/c area-container<%>)
method
(send a-hyper-panel get-canvas%)
 -> (subclass?/c editor-canvas%)
method
(send a-hyper-panel make-control-bar-panel container) -> any/c
  container : (is-a?/c area-container<%>)
method
(send a-hyper-panel rewind) -> void?
method
(send a-hyper-panel forward) -> void?
method
(send a-hyper-panel get-canvas) -> (is-a?/c editor-canvas%)
method
(send a-hyper-panel on-navigate) -> void?
method
(send a-hyper-panel leaving-page page          
                                 new-page) -> any
  page : any/c
  new-page : any/c
method
(send a-hyper-panel filter-notes notes) -> (listof string?)
  notes : (listof string?)
method
(send a-hyper-panel reload) -> void?
class
hyper-panel% : class?
  superclass: (hyper-panel-mixin vertical-panel%)
procedure
(editor->page editor) -> any/c
  editor : (is-a?/c text%)
procedure
(page->editor page) -> (is-a?/c text%)
  page : any/c
parameter
(bullet-size) -> exact-nonnegative-integer?
(bullet-size n) -> void?
  n : exact-nonnegative-integer?
class
image-map-snip% : class?
  superclass: snip%
constructor
(new image-map-snip% [html-text html-text])
 -> (is-a?/c image-map-snip%)
  html-text : (is-a?/c html-text<%>)
method
(send an-image-map-snip set-key key) -> void?
  key : string?
method
(send an-image-map-snip get-key) -> string?
method
(send an-image-map-snip add-area shape      
                                 region     
                                 href)  -> void?
  shape : string?
  region : (listof number?)
  href : string?
struct
(struct exn:cancelled exn ()
    #:extra-constructor-name make-exn:cancelled)
struct
(struct exn:file-saved-instead exn (pathname)
    #:extra-constructor-name make-exn:file-saved-instead)
  pathname : path-string?
value
browser@ : unit?
signature
browser^ : signature
interface
html-text<%> : interface?
  implements: text%
method
(send a-html-text get-url) -> (or/c url? string? false/c)
method
(send a-html-text set-title str) -> void?
  str : string?
method
(send a-html-text add-link start end url) -> void?
  start : exact-nonnegative-integer?
  end : exact-nonnegative-integer?
  url : (or/c url? string?)
method
(send a-html-text add-tag name pos) -> void?
  name : string?
  pos : exact-nonnegative-integer?
method
(send a-html-text make-link-style start     
                                  end)  -> void?
  start : exact-nonnegative-integer?
  end : exact-nonnegative-integer?
method
(send a-html-text add-racket-callback start            
                                      end              
                                      racket-expr) -> void?
  start : exact-nonnegative-integer?
  end : exact-nonnegative-integer?
  racket-expr : string?
method
(send a-html-text add-thunk-callback start      
                                     end        
                                     thunk) -> void?
  start : exact-nonnegative-integer?
  end : exact-nonnegative-integer?
  thunk : (-> any)
method
(send a-html-text post-url  url                   
                           [post-data-bytes]) -> void?
  url : (or/c string? url?)
  post-data-bytes : (or/c bytes? false/c) = #f
mixin
html-text-mixin : (class? . -> . class?)
  argument extends/implements: text%
procedure
(render-html-to-text in             
                     dest           
                     load-img?      
                     eval-rkt?) -> void?
  in : input-port?
  dest : (is-a? html-text<%>)
  load-img? : any/c
  eval-rkt? : any/c
procedure
(send-url str [separate-window?]) -> null
  str : null
  separate-window? : void = #t
procedure
(browser-preference? v) -> boolean?
  v : any/c
procedure
(update-browser-preference url) -> void?
  url : (or/c string? false/c)
procedure
(install-help-browser-preference-panel) -> void?
procedure
(add-to-browser-prefs-panel proc) -> void?
  proc : ((is-a?/c panel%) . -> . any)
value
tool@ : unit?
